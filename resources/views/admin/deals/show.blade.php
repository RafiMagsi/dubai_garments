        ...
        <div class="dg-two-col-grid">
            ...
            <div class="dg-side-stack">
                ...
                <x-ui.card class="dg-panel">
                    ...
                    <form method="POST" action="{{ route('admin.deals.send-email', ['deal' => $deal->id]) }}" class="dg-config-form">
                        @csrf
                        <div class="dg-field">
                            <label class="dg-label" for="recipient_email">Recipient Email</label>
                            <x-ui.input id="recipient_email" name="recipient_email" type="email" :value="$deal->lead?->email" required />
                        </div>
                        <div class="dg-field">
                            <label class="dg-label" for="subject">Subject</label>
                            <x-ui.input id="subject" name="subject" :value="'Update on your order discussion - Deal #'.$deal->id" required />
                        </div>
                        <div class="dg-field">
                            <label class="dg-label" for="message">Message</label>
                            <textarea id="message" name="message" class="dg-textarea" rows="5" required>Hello {{ $deal->lead?->customer_name ?: 'Customer' }},{{ "\n\n" }}We are currently processing your requirements and will share the latest update soon.{{ "\n\n" }}Regards,{{ "\n" }}Dubai Garments Sales Team</textarea>
                        </div>
